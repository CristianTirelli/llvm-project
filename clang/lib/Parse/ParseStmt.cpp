...
#include "clang/Basic/TransformTypeTraits.def"
  ...
  case tok::annot_pragma_loop_cgra:
    ProhibitAttributes(CXX11Attrs);
    ProhibitAttributes(GNUAttrs);
    return ParsePragmaCGRAHint(Stmts, StmtCtx, TrailingElseLoc, CXX11Attrs);
...
StmtResult Parser::ParsePragmaCGRAHint(StmtVector &Stmts,
                                       ParsedStmtContext StmtCtx,
                                       SourceLocation *TrailingElseLoc,
                                       ParsedAttributes &Attrs) {
  ...
  // Get loop hints and consume annotated token.
  while (Tok.is(tok::annot_pragma_loop_cgra)) {
    LoopHint Hint;

    if (!HandlePragmaCGRAHint(Hint))
      continue;
