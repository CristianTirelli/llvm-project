...
std::vector<Value *> getLiveIn(Function *F, graph *g,
                               Instruction *first_loop_inst) {
      ...
      if (Instruction *operand_inst =
        ...
        for (BasicBlock &B : *F) {   // iterate over all the bbs of the function
          for (Instruction &I : B) { // and over all the instructions in the bbs
            if ((&I == first_loop_inst) ||
                first_loop_inst == operand_inst) // stop when you get to the
                                                 // first loop instruction
              before = false;

            if (&I == operand_inst && before &&
...
// return vector only for debug, can remove later
std::vector<Instruction *> getLiveOut(Function *F, graph *g,
  ...
  for (node *n : nodes) {
    ...
    for (User *U : curr->users()) {
      if (Instruction *succ = dyn_cast<Instruction>(U)) {
        if (g->getNode(succ) == nullptr &&
            std::find(liveout.begin(), liveout.end(), curr) == liveout.end() &&
            succ != lastInst) {
...
void addConstants(graph *g) {
  ...
  constant *c;
  nodeId =
      nodeId + 10; //+10 just for debugging purpose can be the usual nodeId++
  for (auto n : nodes) {
    Instruction *inst = dyn_cast<Instruction>(n->getInstruction());
    // if constant is used in icmp to set the flag for a br then add a livein
    // instead
    if (inst->getOpcode() == Instruction::ICmp) {
    ...
    if (inst != nullptr && inst->getOpcode() != Instruction::GetElementPtr &&
        !isa<BranchInst>(inst)) {
        ...
        if (ConstantInt *CI = dyn_cast<ConstantInt>(inst->getOperand(i))) {
          if ((int)CI->getSExtValue() > 4096 ||
            ...
            g->addLiveInEdge(e);
                     ...
                     g->getSuccessors(n).size() == 1) {

            Instruction *succ_n;
            succ_n =
                dyn_cast<Instruction>(g->getSuccessors(n)[0]->getInstruction());
            if (!isa<BranchInst>(succ_n)) {
              ...
              edge *e;
              source =
                  new node(nodeId++, "LiveInConstBr", dyn_cast<Value>(inst));
              destination = g->getNode(inst);
              e = new edge(edgeId++, source, destination, 0);
              g->addLiveInNode(source);
              g->addLiveInEdge(e);
...
void instructionSelection(graph *g) {
  ...
  for (auto n : nodes) {
    ...
    case Instruction::GetElementPtr: {
      ...
      // multiple way to handle this. The base address could retrived by making
      // a load this would reduce the register pressure but on average it will
      // probably be not the best solution, that's why for now we don't care
      // about register pressure and we store the address in a register maybe
      // the can be also handled in the backend

      // getelemntptr node is going to be replaced by one node that compute the
      // offset and one that compute offset + base address
      //
      //  node1 = 4 * index
      //  node2 = base_addr + node1
      //  node2 will have a livein that is addr and an incoming edge that is
      //  node1 n is going to be replaced by node 2 (so that only one assignment
      //  needs to be changed)
      ...
      n->setROp(offset);
      constant *c;
      c = new constant(nodeId++, offset,
                       4); // i*4 should be 1,2 or 4 but for now in the ISA
    ...
    case Instruction::ICmp: {
      ...
      // AND ALWAYS USE A SUB INST INSTEAD (wih this sometimes you will have the
      // same number
        ...
        if (g->getSuccessors(n).size() == 1 &&
          ...
          // (or at least should have)
                   ...
                   dyn_cast<Instruction>(
                       g->getSuccessors(n)[0]->getInstruction())
                           ->getOpcode() == Instruction::Select) {
          n->setInstructionName("sub");
          n->setOpcode(SSUB);
          Instruction *suc_inst =
              dyn_cast<Instruction>(g->getSuccessors(n)[0]->getInstruction());
          ...
          for (auto succ_node : g->getSuccessors(n)) {
            Instruction *succ_inst =
                dyn_cast<Instruction>(succ_node->getInstruction());
              ...
              if (isLess(n) || isGreater(n)) {
                ...
                for (auto e : g->getAssociateEdges(n)) {
                  if (e->getFrom()->getId() == n->getId() &&
                      dyn_cast<Instruction>(e->getTo()->getInstruction())
                              ->getOpcode() != Instruction::Select) {
                    e->setFrom(n1);
                    if (e->getTo()->getLOp() != nullptr &&
                        e->getTo()->getLOp()->getId() == n->getId()) {
                      e->getTo()->setLOp(n1);
                    }
                    if (e->getTo()->getROp() != nullptr &&
                        e->getTo()->getROp()->getId() == n->getId()) {
                      e->getTo()->setROp(n1);
                    }
                    if (e->getTo()->getPredicate() != nullptr &&
                        e->getTo()->getPredicate()->getId() == n->getId()) {
          ...
          //<= can be done with > and a swap of the operands in the next
          // instruction Add nodes and leave br ins becomes beq

          // if <= or >= add two nodes to check each flag and one node to
          //  do the or between them
          // else just add one node that check one of those (depending on which
          // flag)
          /*
...
PreservedAnalyses CGRAExtractPass::run(Loop &L, LoopAnalysisManager &AM,
  ...
  if (nestedLoop.size() == 0) {
    if (hasPragmaCGRAAcc(&L, "llvm.loop.cgra.acc")) {
      ...
      edgeId = 0;
      loop_number++;
      filename = L.getBlocks()[0]->getParent()->getName().str() +
                 std::to_string(loop_number);
      g = new graph();
      flG = filename;
      ...
      std::vector<Value *> livein =
          getLiveIn(L.getBlocks()[0]->getParent(), g, firstInst);
      std::vector<Instruction *> liveout =
          getLiveOut(L.getBlocks()[0]->getParent(), g, lastInst);
      assignOperands(g);
